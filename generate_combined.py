...
with open('gen_hooks.py', 'r') as f:
    gen_content = f.read()

# I will just write a new python script to generate exactly what I need.
...
combined = f\"\"\"    static func foundationalCatalog() -> [HookCapabilityDefinition] {{
        [
{foundational}
{"\\n".join(output)}
        ]
...
with open('scratch/combined_catalog.swift', 'w') as f:
    f.write(combined)
"""
with open('scratch/gen_combined.py', 'w') as f:
    f.write(new_script)
