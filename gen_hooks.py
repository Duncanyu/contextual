...
output = []
for h in hooks:
    # Handle optional permission level
    perm_level = ".none"
    if len(h) > 8:
        perm_level = h[8]
...
with open('hooks.swift', 'w') as f:
    f.write("\n".join(output))

with open('sandbox_cases.swift', 'w') as f:
    cases = []
    for h in hooks:
        name = h[0].replace("_", " ").title().replace(" ", "")
        cases.append(f'        case "{h[0]}": return run{name}(ctx: ctx)')
    f.write("\n".join(cases))

with open('sandbox_stubs.swift', 'w') as f:
    stubs = []
    for h in hooks:
        name = h[0].replace("_", " ").title().replace(" ", "")
        
        if h[1] == "computerControl" or h[1] == "dangerous":
            log = f'print("[AppControlHook] action={h[0]} confirmation_required=yes")'
        elif "llm" in h[0] or "response" in h[0]:
            log = f'print("[LLMHook] model=local_fallback completed elapsed_ms=15")'
        else:
            log = ""
    ...
    private func run{name}(ctx: SandboxContext) -> HookSandboxStepOutcome {{
        {log}
        let output = "{h[0]} stub execution success"
...
with open('safe_ids.swift', 'w') as f:
    ids = []
    for h in hooks:
        ids.append(f'        "{h[0]}",')
    f.write("\n".join(ids))
